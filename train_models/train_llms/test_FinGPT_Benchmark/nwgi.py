...
def label_to_output(example):
    return {"output": dic[example['label']]}

def select_columns(example):
    return {"input": example["input"], "output": example["output"], "instruction": example["instruction"]}

def apply_format_example(example):
    formatted_example = format_example(example)
    return {"context": formatted_example["context"], "target": formatted_example["target"]}
...
def test_nwgi(args, model, tokenizer, prompt_fun=None):
    ...
    #     dataset[split] = dataset[split].map(label_to_output)
    ...
    #     else:
    #         instruction = prompt_fun  # 这里需要根据prompt_fun的实现来调整

    #     dataset[split] = dataset[split].map(lambda example: {"instruction": instruction, "input": example["news"]})


    dataset = dataset[['input', 'output', 'instruction']]
    # for split in dataset.keys():
    #     dataset[split] = dataset[split].map(select_columns)

    dataset[["context","target"]] = dataset.apply(format_example, axis = 1, result_type="expand")
    # for split in dataset.keys():
    #     dataset[split] = dataset[split].map(apply_format_example, batched=False)

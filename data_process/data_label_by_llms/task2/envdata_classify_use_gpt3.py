...
def call_with_messages(text, api_key, temperature=0.5):
    messages = [
        {'role': 'system', 'content': 'You are a expert in ESG data classification, especially environmental ESG data classification'},
        {'role': 'user', 'content': f"To identify environmental data of ESG data, consider the following criteria: \n, \
                Environmental criteria comprise a company’s energy use, waste management (contaminated land or disposal of hazardous/toxic emissions), \
                pollution, natural resource conservation, and treatment of the biosphere, as well as compliance with governmental regulations. \
                Special areas of interest are climate change and environmental sustainability (e.g., handling diminishing raw materials).\n \
                Answer 'Yes' or 'No' first, then give the explanation. \n\n \
                Text: Through the course of 2021, M&G Investments continued to participate in CA100+ collective engagement groups. \n \
                Answer: Yes, CA100+ is a climate initiative. \n \
                Text: Individual investments can have both a positive and negative impact on society and the environment. \n \
    ...
    try: 
        ...
        is_env, explanation = determine_environmental_from_response(response)  # Implement this function based on your response format

        return is_env, explanation, usage
...
def determine_environmental_from_response(response):
        ...
        is_env = explanation.lower().startswith("yes") or "answer: yes" in explanation.lower()
        return is_env, explanation
    ...
    with tqdm(total=total_items, desc="Processing", unit="item") as pbar:
        ...
        for item in data:
            ...
            if item['text'] not in existing_texts:
                is_env, explanation, usage = call_with_messages(item['text'], api_key)
                save_usage_data(usage, usage_file)

                item.update({'is_env': is_env, 'explanation2': explanation})
                # print(item)
